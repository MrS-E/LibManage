document.querySelector("button#submit").addEventListener('click', (e)=>{
    ...
    console.log(updated)
    fetch('/admin/'+window.location.href.split('/')[window.location.href.split('/').length-1].split('?')[0], {method:"put", headers: {"Content-Type": "application/json",}, body: JSON.stringify(updated)})
        .then((res)=>res.json())
        .then((data)=>{
            console.log(data)
